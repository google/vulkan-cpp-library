...
int main(int argc, const char **argv) {
...

	{
		...
		const std::set<std::string> dev_extensions{
	...
	vcc::descriptor_set::update(device,
		...
		vcc::descriptor_set::write_buffer(std::ref(desc_set), 2, 0,
			VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
		...
		vcc::pipeline::vertex_input_state{
			{
				...
				VkVertexInputBindingDescription{ 1,
					sizeof(glm::mat4) + sizeof(glm::vec4) * 3,
	...
	return vcc::window::run(window,
		...
		vcc::memory::bind(std::ref(device), VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
			*depth_image);
			...
			vcc::command_buffer::compile(command_buffers[i], 0, VK_FALSE, 0, 0,
				vcc::command_buffer::render_pass(std::ref(render_pass),
					...
					vcc::command_buffer::bind_descriptor_sets{
						...
						{ std::ref(desc_set) },{} },
					vcc::command_buffer::set_viewport{ 0, { VkViewport{
						0.f, 0.f, float(extent.width), float(extent.height),
						0.f, 1.f
						} } },
					vcc::command_buffer::set_scissor{ 0,
