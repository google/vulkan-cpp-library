...
int main(int argc, const char **argv) {
	...
	vcc::sampler::sampler_type normal_sampler(vcc::sampler::create(
		...
		0, VK_TRUE, 1, VK_FALSE, VK_COMPARE_OP_NEVER, 0, 0, VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE,
		VK_FALSE));
	...
	vcc::window::window_type window(vcc::window::create(
		...
		VK_FORMAT_A8B8G8R8_UINT_PACK32, "Normal mapping and cube texture demo"));
			...
			vcc::pipeline::vertex_input_state{
				...
				},
				{
					...
					VkVertexInputAttributeDescription{ 3, 0, VK_FORMAT_R32G32B32_SFLOAT,
						sizeof(glm::vec4) * 3 },
					VkVertexInputAttributeDescription{ 4, 0, VK_FORMAT_R32G32B32_SFLOAT,
						sizeof(glm::vec4) * 4 }
				}
			},
		...
		for (std::size_t i = 0; i < swapchain_images.size(); ++i) {
			vcc::command::compile(
				...
				vcc::command::render_pass(std::ref(render_pass),
					...
					vcc::command::bind_pipeline{ VK_PIPELINE_BIND_POINT_GRAPHICS,
						std::ref(pipeline) },
					vcc::command::bind_vertex_buffers(0, { std::ref(vertex_buffer) }, { 0, 0 }),
					vcc::command::bind_index_data_buffer(std::ref(index_buffer), 0,
