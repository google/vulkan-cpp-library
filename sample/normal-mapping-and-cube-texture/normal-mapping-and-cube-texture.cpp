...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
	...
	return vcc::window::run(window,
		...
		for (std::size_t i = 0; i < swapchain_images.size(); ++i) {
			vcc::command_buffer::compile(command_buffers[i], 0, VK_FALSE, 0, 0,
				vcc::command_buffer::render_pass(
					...
					vcc::command_buffer::bind_vertex_data_buffers({ std::ref(vertex_buffer) }, { 0, 0 }),
		...
		modelview_matrix[0] = model_view;
		normal_matrix[0] = glm::mat3(glm::transpose(glm::inverse(model_view)));
